...
//        System.out.println(document.getElementsByTag("img").first().attr("src"));
    }

    public static String getTitle(String pageXml){
        try {
            Document document = Jsoup.parse(pageXml);//获取html文档
            return document.getElementsByTag("title").first().html();
        }catch (Exception e){
            return null;
        }
    }

    public static String getFirstImg(String pageXml){
        try {
            Document document = Jsoup.parse(pageXml);//获取html文档
            return document.getElementsByTag("img").first().attr("src");
        }catch (Exception e){
            return null;
        }
    }
